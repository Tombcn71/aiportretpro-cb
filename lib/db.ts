...
export async function createProject(data: {
  ...
  try {
    const result = await sql`
      INSERT INTO projects (user_id, purchase_id, name, gender, outfits, backgrounds, uploaded_photos)
      VALUES (${data.userId}, ${data.purchaseId}, ${data.name}, ${data.gender}, ${data.outfits}, ${data.backgrounds}, ${data.uploadedPhotos})
      RETURNING *
    `
    return result[0]
  } catch (error) {
